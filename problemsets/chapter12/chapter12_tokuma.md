...
1. どの辺を利用するかを非決定的に定める
  - 選んだ始点ごとに、別の辺を利用するか、しないかで分岐する計算木を作成する。各ノードでクローンを生成する。
  - どのケースに置いても、計算木の深さは最大でも$O(n)$である。
2. その辺の集合がハミルトン閉路であるかを次のプロセスでチェックする
  - 与えられた辺の集合がそれぞれ1回のみ使われているか: $O(n)$
  - 始点と終点が同じか: $O(1)$
  - すべてのノードを1度ずつ通っているか: $O(n)$
...
3. 計算結果$M/N$が整数であるかを判定する。: $O(n)$
    - 見るべき数字は$\log (M/N)$桁であるが、これも$n$で抑えられる。
...
3. 適切なものとつながっていれば、yes, そうでなければnoを返す。
...
1. どの辺を利用するかを非決定的に定める
  - 選んだ始点ごとに、別の辺を利用するか、しないかで分岐する計算木を作成する。各ノードでクローンを生成する。
  - どのケースに置いても、計算木の深さは最大でも$O(n)$である。
2. その辺の集合がハミルトン路であるかを次のプロセスでチェックする
  - 与えられた辺の集合がそれぞれ1回のみ使われているか: $O(n)$
  - すべてのノードを1度ずつ通っているか: $O(n)$
3. ハミルトン路であれば、計算木の始点, そうでなければnoを返す。
...
また以下の条件を満たす。
- $I$が正インスタンスの場合: `verifyAdd`は `correct`を返す
- $I$が負インスタンスの場合: `verifyAdd`は`unsure`を返す
- $S$が誤っている場合:`verifyAdd`は`unsure`を返す。
